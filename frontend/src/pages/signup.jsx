    ...
    <div className="min-h-screen bg-[#F7F4EE] flex items-center justify-center px-4">
      <div className="w-full max-w-sm">
        ...
        <form onSubmit={handleSubmit} className="bg-white border border-[#E5E0D5] rounded-sm shadow-sm p-8">
          ...
          <label className="block text-xs font-medium text-[#64748B] uppercase tracking-wide mb-2">Name</label>
          <input
            type="text"
            value={name}
            onChange={(e) => setName(e.target.value)}
            className="w-full border border-[#E5E0D5] rounded-sm px-3 py-2.5 mb-4 text-[#16233B] focus:outline-none focus:border-[#B08D57] transition"
            required
          />
          ...
          <p className="text-sm text-[#64748B] mt-5 text-center">
            ...
            <Link to="/login" className="text-[#B08D57] hover:underline">Log in</Link>
          </p>
        </form>
      </div>
    </div>
